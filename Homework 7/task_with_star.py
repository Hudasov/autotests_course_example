...
class RomanNums:
    ...
    def from_roman(self):
        ...
        for i in range(len(self.number)):
            if i > 0 and roman_dict[self.number[i]] > roman_dict[self.number[i - 1]]:
                arabic_numeral += roman_dict[self.number[i]] - 2 * roman_dict[self.number[i - 1]]
            else:
                arabic_numeral += roman_dict[self.number[i]]
