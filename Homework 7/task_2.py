...
class PersonInfo:
    ...
    def short_name(self):
        return f"{self.name.split()[-1]} {self.name.split()[0][0]}."
    ...
    def new_salary(self):
        ...
        # НЕ ДОЖАЛ my_dict = {c: my_dict.get(c, 0) + 1 for department in self.departments for c in department}

        top_3 = []
        for k in sorted(my_dict.values(), reverse=True):
            if len(top_3) < 3:
                top_3.append(k)

        return 1337 * self.age * sum(top_3)
